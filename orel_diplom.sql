...
Mbps FROM price INNER JOIN specifications ON price.id=specifications.id INNER JOIN quantity ON price.id=quantity.id WHERE Mbps = '576') 
SELECT SUM(quantity) * (SUM(retail_price) - SUM(stock_price)) as income FROM tmp;  -- 4.посчитать количество выручки (retail_price - stock_price), если мы продадим все видеорегистраторы в наличии у которых 576 Mbps
...
WITH tmp AS (SELECT price.id, chanel, AVG(retail_price) OVER (PARTITION BY chanel) as avg_retail_price, AVG(stock_price) OVER (PARTITION BY chanel) as avg_stock_price 
...
SELECT MIN(retail_price) OVER (PARTITION BY Mbps) as min_price, AVG(retail_price) OVER (PARTITION BY Mbps) as avg_price, MAX(retail_price) OVER (PARTITION BY Mbps) as max_price FROM price 
...
FROM sales_report_122018 INNER JOIN customer ON sales_report_122018.id_customer=customer.id_customer ORDER BY sum DESC LIMIT 1; -- 9.определить покупателя, который купил больше всего видеорегистраторов. Вывести название фирмы-покупателя и количество купленных ею видеорегистраторов.
